...
export function ArticleView({ slug, locale = "en" }: { slug: string; locale?: Locale }) {
  ...
  return <SiteShell locale={locale}><main className="article-main">
    ...
    {article.image && <figure className="article-figure"><img src={article.image.src} alt={tx(article.image.alt)} width="1052" height="592" loading="lazy" /><figcaption>{tx(article.image.caption)}</figcaption></figure>}
