...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const homeSeo: Record<Lang, { title: string; description: string }> = {
    en: { title: "Hacoo Spreadsheet & Links 2026: Finds, QC and Shipping", description: "Compare Hacoo spreadsheet links, product finds, QC checks, shipping facts and returns evidence in one independent 2026 decision hub." },
    de: { title: "Hacoo Tabelle & Links 2026: Funde, QC und Versand", description: "Hacoo-Tabellenlinks, Produktfunde, QC-Prüfungen, Versandfakten und Retourenbelege in einem unabhängigen Entscheidungsratgeber vergleichen." },
    es: { title: "Hoja y enlaces Hacoo 2026: hallazgos, QC y envíos", description: "Compara enlaces de la hoja Hacoo, productos, controles QC, datos de envío y pruebas de devolución en una guía independiente de 2026." },
    fr: { title: "Hacoo France 2026 : liens, sélections, livraison et retours", description: "Comparez les liens et sélections Hacoo, les contrôles QC, les prix et délais de livraison ainsi que les conditions de retour dans un guide indépendant." },
    it: { title: "Foglio e link Hacoo 2026: prodotti, QC e spedizione", description: "Confronta link del foglio Hacoo, prodotti, controlli QC, dati di spedizione e prove per i resi in una guida indipendente del 2026." },
  };
  const title = parsed.route === "article" ? a!.title : parsed.route === "" ? homeSeo[parsed.lang].title : isTrustRoute(parsed.route) ? `${trustContent[parsed.lang][parsed.route].title} | Hacoo VIP` : parsed.lang === "en" ? routeSeo[parsed.route].title : `${l.pageInfo[parsed.route].title} | Hacoo VIP`;
  const description = parsed.route === "article" ? a!.excerpt : parsed.route === "" ? homeSeo[parsed.lang].description : isTrustRoute(parsed.route) ? trustContent[parsed.lang][parsed.route].intro : parsed.lang === "en" ? routeSeo[parsed.route].description : l.pageInfo[parsed.route].text;
