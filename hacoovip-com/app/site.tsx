...
const routePaths: RouteKey[] = ["", "spreadsheet", "finds", "qc-guide", "shipping", "returns", "articles", "faq"];
...
function live(url: string, label: string) {
  ...
  target.searchParams.set("utm_campaign", "hacoo-spreadsheet");
...
function Home({ lang }: { lang: Lang }) {
  ...
  return <>
    ...
    <section className="section clay"><Heading index={l.home.findsIndex} title={t.findsTitle} text={t.findsText}/><ProductGrid lang={lang}/><p className="price-note">{l.home.priceNote}</p></section>
    <section className="method"><div><span>{l.home.methodIndex}</span><h2>{t.methodTitle}</h2><p>{t.methodText}</p><a href={localizedPath(lang, "qc-guide")}>{t.nav[3]} →</a></div><div className="steps">{t.steps.map(([title, text], i) => <article key={title}><span>0{i + 1}</span><h3>{title}</h3><p>{text}</p></article>)}</div></section>
...
function PageHero({ lang, route }: { lang: Lang; route: Exclude<RouteKey, ""> }) { const p = localizedContent[lang].pageInfo[route]; return <section className="page-hero"><span>{p.eyebrow}</span><h1>{p.title}</h1><p>{p.text}</p></section>; }
...
function BasicPage({ lang, route }: { lang: Lang; route: Exclude<RouteKey, ""> }) {
  const l = localizedContent[lang];
...
function ArticlePage({ lang, slug }: { lang: Lang; slug: ArticleSlug }) {
  ...
  return <article className="article-page"><header><a href={localizedPath(lang, "articles")}>← {copy[lang].nav[6]}</a><span>{l.pageInfo.articles.eyebrow} · {copy[lang].updated}</span><h1>{a.title}</h1><p>{a.excerpt}</p><div className="article-meta"><b>{l.common.primaryKeyword}</b><span>{a.keyword}</span><b>{l.common.length}</b><span>{l.common.articleLength}</span></div></header><div className="article-note"><b>{l.common.researchNote}</b><p>{a.sourceNote} {l.common.researchSuffix}</p></div><figure className="article-figure"><img src={a.image} alt={a.imageAlt} width="800" height="800"/><figcaption>{a.imageCaption}</figcaption></figure><div className="article-body">{a.sections.map(([title, text], i) => <section key={title}><span>{String(i + 1).padStart(2, "0")}</span><div><h2>{title}</h2><p>{text}</p></div></section>)}</div><aside className="article-cta"><span>{l.common.nextStep}</span><h2>{l.common.testWorkflow}</h2><a href={localizedPath(lang, "spreadsheet")}>{copy[lang].nav[1]} →</a></aside></article>;
}
...
function Schemas({ lang, route, articleSlug }: { lang: Lang; route: RouteKey | "article"; articleSlug?: ArticleSlug }) {
  ...
  const graph: object[] = [{ "@type": "WebSite", "@id": `${DOMAIN}/#website`, name: "Hacoo VIP", url: DOMAIN, inLanguage: lang }, { "@type": "Organization", "@id": `${DOMAIN}/#publisher`, name: "Hacoo VIP Research", url: DOMAIN, logo: `${DOMAIN}/hacoo.png` }, { "@type": "WebPage", name: route === "article" ? la!.title : route === "" ? copy[lang].heroTitle : l.pageInfo[route].title, url, inLanguage: lang, isPartOf: { "@id": `${DOMAIN}/#website` } }];
  ...
  if (route === "article") graph.push({ "@type": "Article", headline: la!.title, datePublished: "2026-08-28", dateModified: "2026-08-28", mainEntityOfPage: url, inLanguage: lang, author: { "@type": "Organization", name: "Hacoo VIP Research" } });
  if (route !== "") graph.push({ "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: l.common.home, item: `${DOMAIN}${localizedPath(lang)}` }, { "@type": "ListItem", position: 2, name: route === "article" ? la!.title : l.pageInfo[route].title, item: url }] });
  return <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ "@context": "https://schema.org", "@graph": graph }) }}/>;
}

function Footer({ lang }: { lang: Lang }) { const l = localizedContent[lang]; return <footer><a className="logo inverse" href={localizedPath(lang)}><img src="/hacoo.png" alt="Hacoo" width="200" height="64"/></a><p>{copy[lang].disclaimer}</p><div><a href={localizedPath(lang, "shipping")}>{l.common.shippingFacts}</a><a href={localizedPath(lang, "returns")}>{l.common.returnsFacts}</a><span>hacoovip.com · 2026</span></div></footer>; }
...
export default function Site({ lang, route, articleSlug, routePath }: { lang: Lang; route: RouteKey | "article"; articleSlug?: ArticleSlug; routePath: string }) {
  const l = localizedContent[lang]; return <main lang={lang}><Schemas lang={lang} route={route} articleSlug={articleSlug}/><Header lang={lang} routePath={routePath}/>{route === "" ? <Home lang={lang}/> : route === "article" ? <ArticlePage lang={lang} slug={articleSlug!}/> : <BasicPage lang={lang} route={route}/>}<Footer lang={lang}/><a className="whatsapp" href={WHATSAPP} target="_blank" rel="noopener" aria-label={l.common.whatsapp}><span>WA</span><b>WhatsApp</b></a></main>;
}
