...
function ArticlePage({ lang, slug }: { lang: Lang; slug: ArticleSlug }) {
  ...
  const updated = slug === "hacoo-use-case-must-have-criteria" ? { en: "Checked 18 September 2026", de: "Geprüft am 18. September 2026", es: "Revisado el 18 de septiembre de 2026", fr: "Vérifié le 18 septembre 2026", it: "Verificato il 18 settembre 2026" }[lang] : copy[lang].updated;
...
function Schemas({ lang, route, articleSlug }: { lang: Lang; route: RouteKey | "article"; articleSlug?: ArticleSlug }) {
  ...
  if (route === "article") { const articleDate = articleSlug === "hacoo-use-case-must-have-criteria" ? "2026-09-18" : articleSlug === "hacoo-product-decision-matrix" ? "2026-08-29" : "2026-08-28"; graph.push({ "@type": "Article", headline: la!.title, description: la!.excerpt, image: `${DOMAIN}${la!.image}`, datePublished: articleDate, dateModified: articleDate, mainEntityOfPage: url, inLanguage: lang, author: { "@type": "Organization", name: "Hacoo VIP Research", url: `${DOMAIN}${localizedPath(lang, "about")}` }, publisher: { "@id": `${DOMAIN}/#publisher` }, keywords: [la!.keyword, "Hacoo product comparison", "Hacoo decision guide"], about: [{ "@type": "Thing", name: "Hacoo product comparison" }, { "@type": "Thing", name: la!.keyword }] }); }
