...
export const pageContent: Record<Section, {
  ...
  spreadsheet: {
    ...
    title: "How to use the Joyagoo spreadsheet without losing the live product context.",
    intro: "A Joyagoo spreadsheet is most useful as a discovery layer. It can organize finds and product links, but the current listing, exact option, order record and later warehouse photos are what turn a promising row into a defensible buying decision.",
    blocks: [
      { number: "01", title: "Choose one category and build a small shortlist", text: "Broad product feeds encourage fast but weak comparisons. Begin with one category—such as shoes, hoodies, accessories or electronics—and define the job the item must do. Save no more than three candidates that can be compared on the same fields. For footwear, those fields may be size evidence, sole shape, box volume and visible construction. For clothing, use garment measurements, fabric description, color, print or embroidery and intended fit. Record the spreadsheet title, destination and check date. A shorter list makes dead links, mismatched previews and missing information much easier to spot than a folder with dozens of undated screenshots.", bullets: ["Compare like with like instead of mixing unrelated categories", "Give every saved route a visible check date", "Mark missing evidence instead of filling gaps with assumptions"] },
      { number: "02", title: "Verify the live product link before relying on it", text: "Open the destination and confirm that the page still represents the product shown in the spreadsheet. Match the product type, main image, distinctive design details and option structure—not only the fact that the page returns successfully. Sellers can remove a listing, replace it, change variations or attach a familiar image to another option. Expand the live selectors and check whether the required color, size, version and quantity are still available. Recheck the selected variation price and Chinese domestic shipping in the current order flow. A spreadsheet price is a browsing reference, not a guaranteed payable total. If the destination redirects to an unrelated product or the desired option has disappeared, remove it from the shortlist rather than choosing the nearest substitute.", bullets: ["Match title, image, category and final destination", "Check option-specific availability and price", "Pause changed or redirected routes until they are reviewed again"] },
      { number: "03", title: "Record the exact option and measurement evidence", text: "Create one line that separates product, color, seller size, version and quantity. Preserve the seller's wording when two variations look similar; an invented translation can turn a correct link into a wrong order. For shoes and clothing, compare the seller chart with an item you already own and label every value with its unit and measurement point. Foot length and insole length are different. A flat garment chest width is different from body circumference. For electronics, record model, plug, voltage, connector and battery details where relevant. Save the live option record before payment so it can later be compared with the order confirmation and warehouse photos. When a critical field is unclear, ask for clarification instead of using the spreadsheet image as proof.", bullets: ["Keep original option text beside any working translation", "Label centimeters, inches and size systems clearly", "Treat the selected variation—not the thumbnail—as the order instruction"] },
      { number: "04", title: "Connect discovery, order, QC and parcel evidence", text: "Keep each kind of evidence in the stage where it belongs. Spreadsheet images help find candidates. Seller listing images describe the offer. The order record proves which variation was submitted. Warehouse QC photos show the specific item received at that time. Review QC by first checking identity, color, size tag, version and quantity, then visible construction and category-specific details. If a decisive area is missing, request one purposeful image or measurement through the current account tools where available. Make any eligible after-sales decision while the live action remains available. Once the item is accepted, add stored weight, packaging choice and restriction notes before parcel submission. This chain prevents an attractive preview from being mistaken for evidence about fit, authenticity, durability or final shipping cost.", bullets: ["Use the QC checklist for the received item", "Keep return timing separate from warehouse storage", "Compare actual and volumetric parcel scenarios before selecting a route"] },
    ],
  },
  finds: {
