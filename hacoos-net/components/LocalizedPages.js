...
function getLocalizedCatalog(locale) {
  ...
  const localizedProducts = products.map((product) => ({
    ...product,
    ...research.products[product.slug],
    name: product.name,
    catalogLabel: product.catalogLabel,
    category: categoryNames[product.categorySlug],
  }));
...
export function LocalizedHome({ locale }) {
  ...
  return <div className="localized-shell home-page club-home" lang={locale}><StructuredData data={schema}/>
    <section className="club-hero"><div className="wrap club-hero-grid"><div className="club-hero-copy"><div className="club-kicker"><span>{copy.home.eyebrow}</span><b>{copy.home.trust[0]}</b></div><h1>{copy.home.title[0]}<br/><em>{copy.home.title[1]} {copy.home.title[2]}</em></h1><p>{copy.home.lead}</p><div className="hero-actions"><Link className="button primary" href={localizePath("/spreadsheet",locale)}>{copy.nav.spreadsheet} <Arrow/></Link><Link className="button quiet" href={localizePath("/categories",locale)}>{copy.home.browse}</Link></div><div className="club-mini-proof">{copy.home.trust.map((item)=><span key={item}>{item}</span>)}</div></div>
      <div className="club-collage"><figure className="club-photo club-photo-main"><ResponsiveImage src="/products/shoe-performance.webp" small="/products/shoe-performance-480.webp" width={750} height={750} sizes="(max-width: 720px) 58vw, 520px" alt={localizedCategories[0].name}/><figcaption>{localizedCategories[0].name} / 01</figcaption></figure><figure className="club-photo club-photo-small"><ResponsiveImage src="/products/live-6427.webp" small="/products/live-6427-480.webp" width={720} height={960} sizes="(max-width: 720px) 32vw, 240px" alt={localizedCategories[6].name}/><figcaption>{localizedCategories[6].name} / 07</figcaption></figure><div className="club-burst"><strong>08</strong><span>{copy.nav.categories}</span></div><div className="club-note"><span>{copy.home.processLabel}</span><b>{copy.home.processText}</b></div></div></div>
...
export function LocalizedCategory({ locale, slug }) {
  ...
  return <div className="localized-shell" lang={locale}><StructuredData data={schema}/><section className="category-hero"><div className="category-hero-image"><ResponsiveImage src={category.image} small={category.imageSmall} width={category.imageWidth} height={category.imageHeight} sizes="(max-width: 720px) 100vw, 50vw" alt={category.name}/></div><div className="category-hero-copy"><span className="section-label">{category.eyebrow} / {page.label}</span><h1>{category.name}<br/><em>{copy.nav.spreadsheet}.</em></h1><p>{category.description}</p><a className="button primary" href={`${DESTINATION}${category.destination}`} target="_blank" rel="noopener noreferrer">{page.live} {category.name} <Arrow/></a></div></section>
    ...
    <section className="verified-product-section"><div className="wrap"><div className="section-heading compact"><div><span className="section-label">{depth.category.productLabel}</span><h2>{depth.category.productTitle}</h2></div><p>{depth.category.reviewed}</p></div><article className="verified-product"><div className="verified-product-image"><ResponsiveImage src={featuredProduct.image} small={featuredProduct.imageSmall} width={featuredProduct.imageWidth} height={featuredProduct.imageHeight} sizes="(max-width: 720px) 100vw, 48vw" alt={featuredProduct.name}/><span>{reviewed}</span></div><div className="verified-product-copy"><div className="verified-product-status"><span></span>#{featuredProduct.listingId}</div><h3>{featuredProduct.name}</h3><p>{featuredProduct.focus}</p><dl><div><dt>{categoryUi.liveCatalog}</dt><dd>{featuredProduct.catalogLabel}</dd></div><div><dt>{depth.category.updateLabel}</dt><dd>{reviewed}</dd></div></dl><div className="hero-actions"><a className="button primary" href={verifiedListing} target="_blank" rel="noopener noreferrer">{depth.category.openListing} <Arrow/></a><a className="button quiet" href={liveSearch} target="_blank" rel="noopener noreferrer">{depth.category.fallback}</a><Link className="button quiet" href={localizePath(`/products/${featuredProduct.slug}`,locale)}>{categoryUi.researchPage}</Link></div><p className="verified-product-disclosure">{depth.category.disclosure}</p></div></article></div></section>
...
export function LocalizedProduct({ locale, slug }) {
  ...
  return <div className="localized-shell" lang={locale}><StructuredData data={schema}/><section className="product-reference-hero"><div className="product-reference-image"><ResponsiveImage src={product.image} small={product.imageSmall} width={product.imageWidth} height={product.imageHeight} sizes="(max-width: 720px) 100vw, 50vw" alt={product.name}/></div><div className="product-reference-copy"><span className="section-label">{category.name} / {productUi.visualReference}</span><h1>{product.name}</h1><p>{category.description}</p><p>{product.focus}</p><div className="hero-actions"><a className="button primary" href={verifiedListing} target="_blank" rel="noopener noreferrer">{depth.category.openListing} <Arrow/></a><a className="button quiet" href={liveSearch} target="_blank" rel="noopener noreferrer">{depth.category.fallback}</a><Link className="button quiet" href={localizePath(`/categories/${product.categorySlug}`,locale)}>{productUi.openGuide}</Link></div><span className="product-reference-note">#{product.listingId} · {productUi.checked} {reviewed}</span></div></section>
...
export function LocalizedGuide({ locale, slug }) {
  ...
  const articleImage = research.image ? `${SITE_URL}${research.image.src}` : `${SITE_URL}/products/shoe-performance.webp`;
  const schema = research.faqs ? { "@context": "https://schema.org", "@graph": [
    { "@type": "Article", "@id": `${pageUrl}#article`, headline: guide.title, description: guide.short, mainEntityOfPage: { "@id": `${pageUrl}#webpage` }, inLanguage: locale, wordCount, image: articleImage, author: { "@type": "Organization", name: "Hacoos Editorial" }, datePublished: "2026-08-27", dateModified: "2026-08-27" },
  ...
  ] } : { "@context": "https://schema.org", "@type": "Article", headline: guide.title, description: guide.short, mainEntityOfPage: absoluteLocalizedUrl(`/guides/${slug}`,locale), inLanguage: locale, wordCount, image: articleImage, author: { "@type": "Organization", name: "Hacoos Editorial" }, datePublished: "2026-08-26", dateModified: "2026-08-27" };
  const sourceUi = {
    es: ["Registro de pruebas", "Fuentes revisadas", "Consultado"],
    fr: ["Registre des preuves", "Sources consultées", "Consulté"],
    de: ["Quellennachweis", "Geprüfte Quellen", "Geprüft"],
    it: ["Registro delle prove", "Fonti esaminate", "Consultato"],
    pt: ["Registo de evidências", "Fontes consultadas", "Consultado"],
  }[locale];
  ...
  return <div className="localized-shell" lang={locale}><StructuredData data={schema}/><article className="article"><header className="article-hero"><div className="wrap article-head"><span className="section-label">{research.kicker}</span><h1>{guide.title}</h1><div className="article-meta"><span>{page.byline}</span><span>{guide.read}</span><span>{wordCount.toLocaleString(locale)} {localized.static.article.words}</span><span>{research.dateLabel||page.reviewed}</span></div><p>{guide.short}</p><p className="complete-article-intro">{research.intro}</p></div></header>
    ...
    <div className="wrap article-body"><aside><span>{copy.nav.guides}</span>{fullSections.map(([title],index)=><a href={`#section-${index}`} key={`${title}-${index}`}>{title}</a>)}<a href="#evidence-check">{depth.guide.label}</a><a href="#quick-process">{page.process}</a><a href="#research-note">{page.reviewed}</a></aside><div className="article-content">
      {research.image&&<figure className="article-figure"><ResponsiveImage src={research.image.src} small={research.image.src.replace(/\.webp$/, "-480.webp")} width={750} height={750} sizes="(max-width: 720px) 100vw, 760px" alt={guide.title}/><figcaption>{research.image.caption||guide.short}</figcaption></figure>}
      ...
      <section id="research-note" className="article-research-note"><span>{localized.static.article.researchNote}</span><p>{research.sourceNote}</p><p>{copy.footer.disclaimer}</p></section><section className="article-sources"><span className="section-label">{sourceUi[0]}</span><h2>{sourceUi[1]}</h2><div>{sources.map(([source,document,checked])=><article key={`${source}-${document}`}><strong>{source}</strong><p>{document}</p><time dateTime={checked === "August 27, 2026" ? "2026-08-27" : "2026-08-26"}>{sourceUi[2]} {checked}</time></article>)}</div></section><div className="article-callout"><h2>{page.continue}</h2><p>{guide.short}</p><Link className="button primary" href={localizePath("/categories",locale)}>{page.browse} <Arrow/></Link></div></div></div></article></div>;
}
